...
public class week292 {
    ...
    public String largestGoodInteger(String num) {
        ...
        for (int i = 0; i < num.length() - 3; i++) {
            ...
            if (flag && s.compareTo(res) > 0) res = s;
        }
    ...
    public boolean dfs(int x, int y) {
        if (m + n - x - y - 2 < stack.size()) return false;
        ...
        if (x == m - 1 && y == n - 1 && stack.isEmpty()) return true;
        if (boundary(x + 1, y)) res = dfs(x + 1, y);
        if (boundary(x, y + 1)) res |= dfs(x, y + 1);
        if (flag) stack.offerLast('(');
        else stack.pollLast();
        return res;
    }
